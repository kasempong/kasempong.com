...
(function () {
  ...
  var cx = W / 2, cy = H / 2;
  var maxR = Math.hypot(W, H) / 2 + 20;

  // Overlay canvas
  var cv = document.createElement('canvas');
  cv.width  = W;
  cv.height = H;
  cv.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;z-index:9999;pointer-events:all;';
  document.body.appendChild(cv);
  var c = cv.getContext('2d');

  // Pre-render starfield offscreen
  var off = document.createElement('canvas');
  off.width = W; off.height = H;
  var oc = off.getContext('2d');

  var bg = oc.createRadialGradient(cx, cy, 0, cx, cy, maxR);
  ...
  for (var i = 0; i < 260; i++) {
    ...
    var sr = Math.random() * 1.8 + 0.2;
    oc.beginPath();
    oc.arc(sx, sy, sr, 0, Math.PI * 2);
    oc.fillStyle = 'rgba(255,255,255,' + (Math.random() * 0.7 + 0.3) + ')';
    oc.fill();
  }

  // Nebula wisps
  [
    { x: W * 0.18, y: H * 0.28, r: W * 0.2,  col: 'rgba(120,0,200,0.09)'  },
    { x: W * 0.76, y: H * 0.67, r: W * 0.16, col: 'rgba(200,0,120,0.07)'  },
    { x: W * 0.5,  y: H * 0.18, r: W * 0.13, col: 'rgba(0,100,200,0.06)'  },
  ].forEach(function (n) {
    var ng = oc.createRadialGradient(n.x, n.y, 0, n.x, n.y, n.r);
  ...
  var EXPAND_MS = 2100, FADE_MS = 700;
  var startTime  = null;
  ...
  function frame(ts) {
    ...
    if (elapsed < EXPAND_MS) {
      var prog    = easeInOut(Math.min(elapsed / EXPAND_MS, 1));
      var portalR = prog * maxR * 0.78;
      ...
      // Glow ring — drawn before we erase so it sits on the starfield edge
      c.beginPath();
      c.arc(cx, cy, portalR + 3, 0, Math.PI * 2);
      var pulse = 0.5 + 0.4 * Math.sin(elapsed / 100);
      c.strokeStyle = 'rgba(180,80,255,' + pulse + ')';
      c.lineWidth   = 8 + 4 * Math.sin(elapsed / 75);
      c.shadowColor = '#cc44ff';
      c.shadowBlur  = 36;
      c.stroke();
      c.shadowBlur  = 0;

      // Erase interior → page shows through
      c.globalCompositeOperation = 'destination-out';
      c.beginPath();
      c.arc(cx, cy, Math.max(0, portalR), 0, Math.PI * 2);
    ...
    } else if (elapsed < EXPAND_MS + FADE_MS) {
      cv.style.opacity = (1 - (elapsed - EXPAND_MS) / FADE_MS).toString();
      requestAnimationFrame(frame);
