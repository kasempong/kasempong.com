...
export class DB {
  constructor(public d1: D1Database) {}
  ...
  async createStatement(stmt: Statement): Promise<void> {
    await this.d1
      ...
      .bind(stmt.id, stmt.source_type, stmt.bank, stmt.period_start, stmt.period_end, stmt.filename, stmt.uploaded_at, stmt.processed_at)
      .run()
  }

  async getStatements(): Promise<Statement[]> {
    const result = await this.d1.prepare(`SELECT * FROM statements ORDER BY uploaded_at DESC`).all<Statement>()
    return result.results
  }

  async markStatementProcessed(id: string): Promise<void> {
    await this.d1.prepare(`UPDATE statements SET processed_at = ? WHERE id = ?`).bind(new Date().toISOString(), id).run()
  }
  ...
  async createTransactions(txns: Transaction[]): Promise<void> {
    const stmts = txns.map((t) =>
      this.d1
        ...
        .bind(t.id, t.statement_id, t.date, t.description, t.amount, t.currency, t.category, t.vendor_name, t.source, t.bank, t.is_reconciled, t.matched_id, t.is_anomalous, t.anomaly_reason, t.created_at)
    )
  ...
  async updateTransactionCategory(id: string, category: string, vendorName?: string): Promise<void> {
    await this.d1.prepare(`UPDATE transactions SET category = ?, vendor_name = COALESCE(?, vendor_name) WHERE id = ?`).bind(category, vendorName ?? null, id).run()
  }

  async updateTransactionFlags(id: string, flags: { is_reconciled?: number; matched_id?: string | null; is_anomalous?: number; anomaly_reason?: string | null }): Promise<void> {
    const sets: string[] = []
    const vals: unknown[] = []
  ...
  async getVendorCache(pattern: string): Promise<VendorCache | null> {
    return await this.d1.prepare(`SELECT * FROM vendor_cache WHERE pattern = ?`).bind(pattern).first<VendorCache>()
  }

  async upsertVendorCache(entry: VendorCache): Promise<void> {
    await this.d1.prepare(`INSERT OR REPLACE INTO vendor_cache (pattern, vendor_name, category, researched_at) VALUES (?, ?, ?, ?)`).bind(entry.pattern, entry.vendor_name, entry.category, entry.researched_at).run()
  }

  async createInsight(insight: Insight): Promise<void> {
    await this.d1.prepare(`INSERT INTO insights (id, type, title, body, severity, date_start, date_end, transaction_ids, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(insight.id, insight.type, insight.title, insight.body, insight.severity, insight.date_start, insight.date_end, insight.transaction_ids, insight.created_at).run()
  }

  async getInsights(): Promise<Insight[]> {
    const result = await this.d1.prepare(`SELECT * FROM insights ORDER BY created_at DESC LIMIT 50`).all<Insight>()
    return result.results
  }
