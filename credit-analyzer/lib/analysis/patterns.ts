...
// Detect recurring transactions (same merchant, monthly)
export function detectPatterns(transactions: Transaction[]): RecurringPattern[] {
  const groups: Record<string, Transaction[]> = {}

  for (const t of transactions) {
    // Normalize description for grouping (strip transaction IDs, dates from merchant codes)
  ...
  for (const [, txns] of Object.entries(groups)) {
    ...
    // Likely subscription: consistent amount + monthly cadence
    const likelySubscription =
      amountVariance &&
      months.length >= 2 &&
      isMonthlyPattern(months)
    ...
    patterns.push({
      ...
      months,
      likely_subscription: likelySubscription,
    })
  }
...
// Compute daily spending totals for heatmap calendar
...
// Monthly totals by category for stacked bar chart
export function monthlyByCategory(
  transactions: Transaction[]
): Array<{ month: string; [category: string]: number | string }> {
