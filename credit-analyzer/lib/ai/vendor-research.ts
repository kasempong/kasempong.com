...
interface VendorInfo { vendor_name: string; category: string; description: string }

export async function researchVendor(rawDescription: string, apiKey: string): Promise<VendorInfo> {
...
{"vendor_name":"<clean name>","category":"<Food & Dining|Transport|Shopping|Health & Medical|Entertainment|Bills & Utilities|Travel|Education|Personal Care|Other>","description":"<1 sentence about this business>"}`
  ...
  try {
    ...
    return { vendor_name: String(parsed.vendor_name ?? rawDescription), category: String(parsed.category ?? "Other"), description: String(parsed.description ?? "") }
  } catch {
    return { vendor_name: rawDescription, category: "Other", description: "" }
  }
}
