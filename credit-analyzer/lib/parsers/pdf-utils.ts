...
// Parse Thai/English date strings to ISO format
export function parseDateToISO(dateStr: string, referenceYear?: number): string | null {
  ...
  // DD Thai_MMM YY/YYYY
  const thaiMatch = dateStr.match(/^(\d{1,2})\s*([ก-์]+\.?[ก-์]*\.?)\s*(\d{2,4})$/)
  if (thaiMatch) {
    ...
    if (month) {
      ...
      if (year < 100) year += year > 50 ? 1900 : 2000
      if (year > 2400) year -= 543
      return `${year}-${String(month).padStart(2, "0")}-${String(parseInt(thaiMatch[1])).padStart(2, "0")}`
    }
  }
...
// Parse amount strings with commas and negative signs
...
export function detectBank(text: string): string {
  ...
  if (upper.includes("KASIKORNBANK") || upper.includes("KBANK") || text.includes("กสิกรไทย") || text.includes("กสิกร")) return "KBANK"
  if (upper.includes("SCB") || text.includes("ไทยพาณิชย์")) return "SCB"
  if (upper.includes("BANGKOK BANK") || upper.includes("BBL") || text.includes("กรุงเทพ")) return "BBL"
  if (upper.includes("KRUNGSRI") || upper.includes("BAY") || text.includes("กรุงศรี")) return "KRUNGSRI"
