...
export async function parseMmbak(buffer: ArrayBuffer): Promise<MmbakTransaction[]> {
  ...
  const SQL = await initSqlJs({ locateFile: (file: string) => `/sql.js/${file}` })
  ...
  const txnTableName = tables.find((t) => /transaction|record|expense|spending/i.test(t)) ?? tables[0]

  if (!txnTableName) { db.close(); return [] }
  ...
  const results: MmbakTransaction[] = []
  if (rows[0]) {
...
function parseMMDate(raw: string): string | null {
  ...
  if (!isNaN(num) && num > 0) {
    ...
    const d = new Date(ms)
    if (d.getFullYear() > 1990) return d.toISOString().substring(0, 10)
  }
...
export function toStandardTransactions(rows: MmbakTransaction[]): (ParsedTransaction & { category: string })[] {
  return rows.map((r) => ({ date: r.date, description: r.description, amount: r.amount, currency: r.currency, category: r.category }))
}
