...
export default function DashboardPage() {
  ...
  const usedCategories = CATEGORIES.filter((c) =>
    monthlyData.some((m) => (m[c] as number) > 0)
  )
  ...
  for (const t of transactions) {
    ...
    const c = t.category ?? "Other"
    allCatTotals[c] = (allCatTotals[c] ?? 0) + t.amount
  }
  const donutData = Object.entries(allCatTotals)
    .sort(([, a], [, b]) => b - a)
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <SummaryCard
          ...
          sub={`${thisMonthTxns.length} transactions`}
        ...
        <SummaryCard
          ...
          icon={
            <div
              ...
              style={{ backgroundColor: CATEGORY_COLORS[topCategory?.[0] ?? ""] ?? "#94a3b8" }}
            />
          }
        />
        <SummaryCard
        ...
        <SummaryCard
          title="Untracked"
          value={String(recon.untracked)}
          sub={`${recon.pct_tracked}% of CC tracked`}
          icon={<GitMerge className="w-5 h-5 text-yellow-500" />}
          variant={recon.untracked > 0 ? "warning" : "default"}
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2">
          ...
          <CardContent>
            <MonthlyTrends data={monthlyData} categories={usedCategories as string[]} />
          </CardContent>
        </Card>
              ...
              {recentAnomalies.map((t) => (
                <div key={t.id} className="flex items-start justify-between py-2 px-3 rounded hover:bg-gray-50 gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    {t.anomaly_reason && (
                      <p className="text-xs text-gray-500 mt-0.5 truncate">{t.anomaly_reason}</p>
                    )}
                  </div>
