...
interface AnomalyCalendarProps {
  dailyTotals: Record<string, number>    // date -> total spend
...
export function AnomalyCalendar({ dailyTotals, anomalousDays = new Set() }: AnomalyCalendarProps) {
  ...
  return (
    <div className="space-y-6">
              ...
              <div key={wi} className="grid grid-cols-7 gap-1">
                {week.map((cell, di) =>
                  !cell ? (
                    ...
                    <div
                      key={cell.date}
                      className="relative aspect-square rounded cursor-pointer transition-transform hover:scale-110"
                      style={{
                        backgroundColor: getColor(cell.amount, maxAmount),
                        outline: anomalousDays.has(cell.date) ? "2px solid #ef4444" : undefined,
                      }}
      ...
      {tooltip && tooltip.amount > 0 && (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-xs px-3 py-2 rounded-lg pointer-events-none z-50 shadow-lg">
          {tooltip.date}: {formatCurrency(tooltip.amount)}
        </div>
      )}
